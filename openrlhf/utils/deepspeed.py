...
class SpecificDistributedSampler(Sampler):
    def __init__(self, data_source, num_replicas=None, rank=None, shuffle=False, seed=0):
        self.data_source = data_source
        self.num_replicas = num_replicas if num_replicas is not None else dist.get_world_size()
        self.rank = rank if rank is not None else dist.get_rank()
        self.shuffle = shuffle
        self.seed = seed
        self.num_samples = len(data_source) // self.num_replicas
    ...
    def __iter__(self):
        ...
        # Compute the indices for the current rank
...
class DeepspeedStrategy(ABC):
    ...
    def setup_dataloader(
        ...
        if sampler == "Distributed":
            sampler = SpecificDistributedSampler(
                replay_buffer,
